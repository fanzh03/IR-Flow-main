...
def get_next_batch(data_iter, dataloader):
    try:
        val_data = next(data_iter)
    except StopIteration:
        data_iter = iter(dataloader)
        val_data = next(data_iter)
    return val_data, data_iter
...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        ...
        if opt["use_tb_logger"] and "debug" not in opt["name"]:
            ...
            log_dir = "{}/log/{}/tb_logger/".format(opt["path"]["experiments_root"], opt["name"])
            print(log_dir)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            timesteps = sde.generate_random_timesteps(gt=GT)
            model.feed_data(LQ, GT)  # x_start, gt
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    model.test(sde, idx, timesteps_ode)
                    visuals = model.get_current_visuals()

                    in_img = util.tensor2img(visuals["Input"].squeeze())  # uint8
                    gt_img = util.tensor2img(visuals["GT"].squeeze())  # uint8
                    out_value = None

                    for key in visuals["Output"]:
                        out_value = util.tensor2img(visuals["Output"][key].squeeze())  # uint8
                    # calculate PSNR
                    avg_psnr += util.calculate_psnr(out_value, gt_img)
                ...
                if avg_psnr > best_psnr:
                    ...
                    logger.info(f"Saving the best model.current_step:{current_step}")
                    print(f"Saving the best model.current_step:{current_step}")
                    model.save("best")

                # log
                logger.info("# Validation current_PSNR: {:.6f}, Best PSNR: {:.6f}| Iter: {}".format(avg_psnr, best_psnr, best_iter))
                print("# Validation current_PSNR: {:.6f}, Best PSNR: {:.6f}| Iter: {}".format(avg_psnr, best_psnr, best_iter))
                logger_val = logging.getLogger("val")  # validation logger
                logger_val.info("<epoch:{:3d}, iter:{:8,d}, psnr: {:.6f}".format(epoch, current_step, avg_psnr))
                print("<epoch:{:3d}, iter:{:8,d}, psnr: {:.6f}".format(epoch, current_step, avg_psnr))
                # tensorboard logger
                if opt["use_tb_logger"] and "debug" not in opt["name"]:
